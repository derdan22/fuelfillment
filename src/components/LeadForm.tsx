...
export function LeadForm({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <form
      id="lead-form"
      onSubmit={onSubmit}
      className={`relative glass rounded-[2rem] ${compact ? "p-6 sm:p-8" : "p-8 sm:p-10"}`}
    >
      <div className="mb-6">
      ...
      <div className={`grid gap-4 ${compact ? "" : "sm:grid-cols-2"}`}>
        ...
        <div className={compact ? "" : "sm:col-span-2"}>
          <label className="mb-1.5 block text-sm font-medium text-foreground">Коментар</label>
          <textarea
            name="comment"
            rows={4}
            placeholder="Розкажіть коротко про обсяги або задачу"
            className="w-full rounded-2xl border border-border bg-white/80 px-4 py-3 text-sm outline-none transition focus:border-brand focus:ring-4 focus:ring-brand/10"
          />
        </div>
      </div>
      ...
      <button
        ...
        className="mt-6 inline-flex w-full items-center justify-center rounded-2xl bg-brand px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-brand/25 transition hover:-translate-y-0.5 hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-70"
      >
